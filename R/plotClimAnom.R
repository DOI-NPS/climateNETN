  ...
  clim_dat <-
    if(length(new_dates) == 0 || all(is.na(new_dates))){clim_dat_long
      ...
      new_clim <- purrr::map(new_years, function(y){
        suppressWarnings(getClimNOAA(park = park, year = y, months = new_months))}
      ) |> list_rbind()
      if(nrow(new_clim) > 0){
...
anomplot <-
  ggplot(clim_comb4, aes(x = date2, y = anom,
    ...
    labs(x = NULL, y = ylabel,
         alt = paste0("A bar plot of monthly climate anomalies of ", parameter, 
                      " compared to baseline normals for ", park, 
                      " color-coded by whether a month was above or below average, and covering ",
